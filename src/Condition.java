...
//   assumes all risk of use.
	...
	// requires
	//   Input model stream is open and ready to read first line after "<requires>" or "<guarantees>"
	// guarantees
	//   returns a populated instance of condition (or null, if nothing/error in model file)
	...
	// Accessors
		...
		// description
		//   a PIM Overlay operation that returns whether this condition is assert-able or not
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay operation that returns whether this condition has an assertion message or not
		// requires
		//   none
		// guarantees
...
//   Everything from here down is specific to the model compiler itself
		...
		//   the assertion for this Condition has been emitted
			if( this.isAssertable() ) {
				JavaOutput.print( "assert( " + NameService.formatActionStmt( pIMOverlayAssertExpression ) + " )" );
				if( this.hasAssertionMessage() ) {
					JavaOutput.print( ": \"" + NameService.formatActionStmt( pIMOverlayAssertMessage ) + "\"" );
				}
				JavaOutput.println( ";" );
				JavaOutput.indent();
			}
