...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// requires
		//   Input model stream is open and ready to read at first line after "<associationparticipation>"
		//   Context.model() <> null
		//   lower bound is parse-able ( normally '0' or '1', but could be some other positive int )
		//   upper bound is parse-able ( normally '1' or some other positive int. use '-1' to mean many (*) )
		// guarantees
		//   returns a populated instance of association participation (or null, if nothing/error in model file)
	...
	// Constructor(s)
		...
		// requires
		//   aClass <> null
		//   while not truly required (yet), aLowerBound should be <= anUpperBound (beware, -1 == 'many')
		// guarantees
	...
	// Accessors
		...
		// description
		//   a meta-model operation that returns the role name for this Association Participation
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the participating class for this Association Participation
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the lower bound for this Association Participation
		// requires
		//   none
		// guarantees
		//   returns lower bound
		...
		// description
		//   a meta-model operation that tells whether the lower bound for this Association Participation is zero
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the upper bound for this Association Participation
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that tells whether the upper bound for this Association Participation is many
		// requires
		//   none
		// guarantees
	...
	// Modifiers
		...
		// description
		//   a meta-model operation that overwrites the role name for this Association Participation
		// requires
		...
		// description
		//   a meta-model operation that overwrites the lower bound for this Association Participation
		// requires
		...
		// description
		//   a meta-model operation that overwrites the upper bound for this Association Participation
		// requires
	...
	//   Everything from here down is specific to the model compiler itself
		...
		// description
		//   This rule emits the private instance variable for an association participation
		//
		...
		//   the private inst var declaration has been emitted
			...
			if( !this.isUpperBoundMany() ) {
				JavaOutput.print( "private " );
			} else {
				JavaOutput.print( "private ArrayList<" );
			}
			JavaOutput.print( NameService.asClassLevelName( participatingClass.name() ) );
			if( !this.isUpperBoundMany() ) {
				JavaOutput.print( " " );
			} else {
				JavaOutput.print( "> " );
			}
			JavaOutput.println( NameService.asInstanceLevelName( roleName ) + ";" );
		}
		
		public void ruleInitializeParticipationInstVar() {
		// description
		//   This rule emits the instance variable initialization for an association participation
		//
		...
		// guarantees
		//   the private inst var declaration has been emitted
			JavaOutput.print( NameService.asInstanceLevelName( roleName ) );
			if( !this.isUpperBoundMany() ) {
				JavaOutput.println( " = null;" );
			} else {
				JavaOutput.print( " = new ArrayList<" );
				JavaOutput.print( NameService.asClassLevelName( participatingClass.name() ) );
				JavaOutput.println( ">();" );
			}
			JavaOutput.indent();
		...
		// description
		//   This rule emits the participation invariants check for an association participation
		//
		//    AssociationParticipation.#PARTICIPATION_INVARIANTS_CHECK -->
		//			if( lower bound is 1 or greater ) {
		//				'assert ' + <participation role name> +
		//				if( upper bound is 1 ) {
		//					' != null: the multiplicity for association role ' + participation role name + ' is out of range (=0);'		 
		//				} else {
		//					'.size() >= ' + lower bound +
		...
		//   the participation invariants check has been emitted
			if( lowerBound > 0 ) {
				JavaOutput.indent();
				JavaOutput.print( "assert " + NameService.asInstanceLevelName( roleName ) );
				if( upperBound == 1 ) {
					JavaOutput.println( " != null: \"the multiplicity for association role '" + roleName + "' is out of range (=0)\";" );
				} else {
					JavaOutput.print( ".size() >= " + lowerBound + ": \"the multiplicity for association role '" + roleName );
					JavaOutput.println( "' is out of range (<" + lowerBound + ")\";" );
				}
			}
			if( upperBound > 1 ) {
				JavaOutput.indent();
				JavaOutput.print( "assert " + NameService.asInstanceLevelName( roleName ) );
				JavaOutput.print( ".size() <= " + upperBound + ": \"the multiplicity for association role '" + roleName );
				JavaOutput.println( "' is out of range (>" + upperBound + ")\";" );
			}
		}

		public void ruleBinaryNonReflexiveParticipationReferenceCheck() {
		// description
		//   This rule emits the participation reference check for a binary non-reflexive association participation
		//
		...
		//   the participation reference check has been emitted
			...
			if( upperBound == 1 ) {
				if( lowerBound != 1 ) {
					JavaOutput.indent();
					JavaOutput.println( "if( " + NameService.asInstanceLevelName( roleName ) + " != null ) {" );
					JavaOutput.indentMore();
				}
				JavaOutput.indent();
				JavaOutput.println( NameService.asInstanceLevelName( roleName ) + "." + NameService.asInstanceLevelName( otherRoleName ) + "ReferenceCheck( this );" );
				if( lowerBound != 1 ) {
				...
				JavaOutput.indent();
				JavaOutput.println( "for( " + NameService.asClassLevelName( theClass.name() ) + " a" + NameService.asClassLevelName( theClass.name() ) + ": " + NameService.asInstanceLevelName( roleName ) + " ) {" );
				JavaOutput.indentMore();
				JavaOutput.indent();
				JavaOutput.println( "a" + NameService.asClassLevelName( theClass.name() ) + "." + NameService.asInstanceLevelName( otherRoleName ) + "ReferenceCheck( this );" );
				JavaOutput.indentLess();
				JavaOutput.indent();
				JavaOutput.println( "}" );
			}
		...
		// description
		//   This rule emits the participation reference check for a binary reflexive association participation
		//
		...
		// description
		//   This rule emits the participation reference check for an association participation
		//
		//    AssociationParticipation.#IMPLEMENT_REFERENCE_CHECK -->
		//       'public void ' + role name + 'ReferenceCheck( ' + <thatClass> + ' a' + <thatClass> + ' ) {'
		//       if is verbose
		//          '// requires'
		//          '//   a' + <thatClass> + ' != null'
		//          '// guarantees'
		...
		//   the participation reference check has been emitted
			JavaOutput.indent();
			JavaOutput.print( "public void " + NameService.asInstanceLevelName( roleName ) + "ReferenceCheck( " + NameService.asClassLevelName( participatingClass().name() ) );
			JavaOutput.println( " a" + NameService.asClassLevelName( participatingClass().name() ) + " ) {" );
			if( Context.model().isVerbose() ) {
				JavaOutput.indent();
				JavaOutput.println( "// requires" );
				JavaOutput.indent();
				JavaOutput.println( "//   a" + NameService.asClassLevelName( participatingClass().name() ) + " != null" );
				JavaOutput.indent();
				JavaOutput.println( "// guarantees" );
				JavaOutput.indent();
				JavaOutput.println( "//   assertion failure when this object does not have a reference back to the calling object" );
			}
			JavaOutput.indentMore();
			JavaOutput.indent();
			JavaOutput.print( "assert " + NameService.asInstanceLevelName( roleName ) );
			if( upperBound == 1 ) {
				JavaOutput.print( " == a" + NameService.asClassLevelName( participatingClass().name() ) );
			} else {
				JavaOutput.print( ".indexOf( a" + NameService.asClassLevelName( participatingClass().name() ) + " ) != -1 " );
			}
			JavaOutput.println( ": \"this object does not have a reference back to the calling object\";" );
			JavaOutput.indentLess();
			JavaOutput.indent();	
			JavaOutput.println( "}" );
			JavaOutput.println( "" );
		...
		// description
		//   This rule emits the public link service operation for an association participation
		//     this rule looks messy because of all of the generated names floating around, however
		...
		//   Context.associationParticipation() <> null (the association participation being worked with)
		// guarantees
		//   the public link service been emitted
			JavaOutput.indent();
			String otherRoleName = Context.associationParticipation().roleName();
			JavaOutput.print( "public void link" + NameService.asClassLevelName( otherRoleName ) );
			JavaOutput.print( "( " + NameService.asClassLevelName( participatingClass.name() ) );
			JavaOutput.println( " a" + NameService.asClassLevelName( participatingClass.name() ) + " ) {" );
			if( Context.model().isVerbose() ) {
				JavaOutput.indent();
				...
				JavaOutput.println( "// guarantees" );
				JavaOutput.indent();
				JavaOutput.println( "//   both this and a" + NameService.asClassLevelName( participatingClass.name() ) + " are linked to each other" );
			}
			JavaOutput.indentMore();
			if( Context.model().isAssertionsOn() ) {
				JavaOutput.indent();
				JavaOutput.println( "assert( a" + NameService.asClassLevelName( participatingClass.name() ) + " != null );" );
			}
			JavaOutput.indent();
			JavaOutput.print( NameService.asInstanceLevelName( roleName ) );
			if( !this.isUpperBoundMany() ) {
		...
		// description
		//   This rule emits the public unlink service operation for an association participation
		//     this rule looks messy because of all of the generated names floating around, however
		//        the code it actually produces is reasonably straightforward
		//
		...
		//   the public unlink service been emitted
			JavaOutput.indent();
			String otherRoleName = Context.associationParticipation().roleName();
			JavaOutput.print( "public void unlink" + NameService.asClassLevelName( otherRoleName ) );
			JavaOutput.print( "( " + NameService.asClassLevelName( participatingClass.name() ) );
			JavaOutput.println( " a" + NameService.asClassLevelName( participatingClass.name() ) + " ) {" );
			if( Context.model().isVerbose() ) {
				JavaOutput.indent();
				...
				JavaOutput.println( "// guarantees" );
				JavaOutput.indent();
				JavaOutput.println( "//   this and a" + NameService.asClassLevelName( participatingClass.name() ) + " are unlinked" );
			}
			JavaOutput.indentMore();
			if( Context.model().isAssertionsOn() ) {
				JavaOutput.indent();
			...
			if( !this.isUpperBoundMany() ) {
				JavaOutput.indent();
				JavaOutput.println( NameService.asInstanceLevelName( roleName ) + " = null;" );
			} else {
				JavaOutput.indent();
				JavaOutput.print( "int index = " + NameService.asInstanceLevelName( roleName ) );
				JavaOutput.println( ".indexOf( a" + NameService.asClassLevelName( participatingClass.name() ) + " );" );
				JavaOutput.indent();
				JavaOutput.println( "if( index != -1 ) {" );
				JavaOutput.indentMore();
				JavaOutput.indent();
				JavaOutput.println( NameService.asInstanceLevelName( roleName ) + ".remove( index );" );
				JavaOutput.indentLess();
				JavaOutput.indent();
				JavaOutput.println( "}" );
			}
