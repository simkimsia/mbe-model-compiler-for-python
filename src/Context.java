...
//   none
// Application notes
//   I'm not happy with how tightly coupled this makes everything, but the alternative
...
//         deeper embedded rules can now refer to Context.<thing>() to get easy reference
//         Inner rule needs to Context.clear<Thing>() on exit
//         Context.clear<Thing>() is important to avoid residual values causing problems later
//   When there are no further/deeper embedded rules, should be safe to not set context
//   Note: there are lots of violations of Law of Demeter on the getters, but hey, that's life...:
