...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// requires
		//   Input model stream is open and ready to read at the first line after "<attribute>"
		// guarantees
		//   returns a populated instance of attribute (or null, if nothing/error in model file)
            ...
            while ( !line.contains( Attribute.tagAttributeEnd ) ) {
            	...
            	if( line.contains( Attribute.tagAttributeRangeType ) ) {
	                ...
	                if( newAttribute.range() == null ) {
		                System.out.println( " ***** Cannot find declared Range for Attribute " + newAttribute.name() + " on class " + Context.mMClass().name() );
		            }
            	}
	...
	// Accessors
		...
		// description
		//   a PIM Overlay operation that returns whether this attribute is assert-able or not
		// requires
		//   none
		// guarantees
...
//   Everything from here down is specific to the model compiler itself
		...
		//   the instance variable declaration code for this attribute has been emitted
			JavaOutput.indent();
			JavaOutput.print( "private " + definedRange.pIMRunTimeType() );
			JavaOutput.print( " " + NameService.asInstanceLevelName( name ) );
			if( definedRange.pIMInitialValue() != "" ) {
				JavaOutput.print( " = " + definedRange.pIMInitialValue() );
			}			
			JavaOutput.println( ";" );
		}
		...
		//   the public instance variable getter code for this attribute has been emitted
			JavaOutput.indent();
			JavaOutput.print( "public " + definedRange.pIMRunTimeType() );
			JavaOutput.println( " " + NameService.asInstanceLevelName( name ) + "() {" );
			if( Context.model().isVerbose() ) {
				...
				JavaOutput.indent();
				JavaOutput.println( "//   returns the " + name );			
			}
			JavaOutput.indentMore();
			JavaOutput.indent();
			JavaOutput.println( "return " + NameService.asInstanceLevelName( name ) + ";" );
		...
		// description
		//   This rule emits an instance variable invariants check for an attribute, if any
		//
		...
		//   the instance variable declaration code for this attribute has been emitted
			JavaOutput.indent();
			JavaOutput.print( "assert " );
			JavaOutput.print( definedRange.pIMAssertionExpression( NameService.asInstanceLevelName( name ) ) );
			JavaOutput.println( ": \"the value for attribute '" + name + "' is out of range\";" );
		}
