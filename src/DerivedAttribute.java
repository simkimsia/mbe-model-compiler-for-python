...
//   even though it's not shown in the meta-model (because it is PIM Overlay level stuff),
//      Derived Attributes are specified in terms of an Action. This is to give the full power of
...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// requires
		//   Input model stream is open and ready to read at the first line after "<derivedattribute>"
		// guarantees
		//   returns a populated instance of derived attribute (or null, if nothing/error in model file)
	...
	// Accessors
		...
		// description
		//   a meta-model operation that returns the derivation action for this derived attribute
		// requires
		//   none
		// guarantees
...
//  Everything from here down is specific to the model compiler itself
		...
		// guarantees
		//   the public compute-on-demand derived attribute getter code for this attribute has been emitted
			JavaOutput.indent();
			JavaOutput.print( "public " + NameService.formatActionStmt( derivedDataType.pIMRunTimeType() ) );
