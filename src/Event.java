...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// guarantees
		//   returns a populated instance of event (or null, if nothing/error in model file)
		//   and input model string is right after "</event>"
            ...
            if( line.contains( Action.tagPIMReturnDataType ) ) {
                ...
                if( newEvent.returnRange() == null ) {
                   System.out.println( " ***** Cannot find declared Range for Event " + newEvent.name() + " on class " + Context.mMClass().name() );
                }
                line = JALInput.nextLine();
        	}
	...
	//   Everything from here down is specific to the model compiler itself
		...
		//   the operation and method code for this Event have been emitted
			...
			JavaOutput.indent();
			JavaOutput.print( "public " + NameService.asClassLevelName( Context.mMClass().name() ) );
			...
			JavaOutput.println( NameService.asInstanceLevelName( Context.mMClass().name() ) + "Set.add( this );" );
		...
		// description
		//   This rule emits the push-event operation signature and method implementation code for this Event
		...
		//   the operation and method code for this Event have been emitted
			...
			if( returnRange != null ) {
				JavaOutput.print( returnRange.pIMRunTimeType() + " aResult" );
				if( returnRange.pIMInitialValue() != null ) {
					JavaOutput.print( " = " + returnRange.pIMInitialValue() );
				} else {
					System.out.println();
			...
			if( returnRange != null ) {
				JavaOutput.println( "return aResult;" );
			}
			JavaOutput.indentLess();
			JavaOutput.indent();
			JavaOutput.println( "}" );
		...
		//   Context.event == this
		// guarantees
		//   the formal parameters list, including parens for this operation has been emitted
		// Implementation notes
		...
		//            foreach unique requires condition in all actions on all transitions for this event
		//                 '//    ' + that condition's expression
		//            otherwise '//    none'
		//
		// NOTE: there's a potential logic issue with doing contracts this way. Say there is some event e1.
		//       assume states s1 and s2 both have outgoing transitions on e1. The s1-e1 transition has
		//       action a1 which requires p. the s2-e1 transition has action a2 which requires !p. now
		//       operation e1() actually requires both p and !p, which can never be true. to be 100%
		//       theoretically correct, operation e1() contract should probably look more like:
		//
		//       public void e1() {
		...
		//   operation contract requires clause has been emitted
			...
			if( !requiresConditionSet.isEmpty() ) {
				for( Condition aCondition: requiresConditionSet ) {
					JavaOutput.indent();
					JavaOutput.println( "//    " + aCondition.expression() );
				}
			} else {
				JavaOutput.indent();
		...
		//				}
		//
		// NOTE: minor issue in current version is that it will put '-- or --' between any pair of
		//       state-event behaviors, but Ignore types don't output anything. Maybe solution is
		//       to only be explicit about transitions and Impossible, default to Ignore (implicit)
		//
		// requires
		//   Context.class <> null
		//   Context.event == this
