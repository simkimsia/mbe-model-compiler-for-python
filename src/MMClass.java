...
//   This class had to be named MMClass because Java won't allow a class named Class
...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// requires
		//   the model input stream is open and ready to read at the first line after a "<class>" tag
		// guarantees
		//   returns a populated instance of MMClass (or empty instance, if nothing or there is an error in the stream)
		...
		// description
		//   a meta-model operation that tries to find a class named the same as aClassName
		// requires
		//   aClassName <> null
		// guarantees
	...
	// Constructor(s)
		...
		//     NOTE: will eventually have to implement a unique-ness check, but haven't done it yet
		// guarantees
	...
	// Accessors
		...
		// description
		//   a meta-model operation that tries to find an attribute named the same as anAttributeName
		// requires
		//   an attribute name <> null
		// guarantees
		//   (a reference to) the attribute with the same name as anAttributeName has been returned
		...
		// description
		//   a model compiler helper operation that tries to return the state attribute enum type on this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that tries to find an action named the same as anActionName
		// requires
		//   none
		// guarantees
		//   (a reference to) the action with the same name as anActionName has been returned
		//     returns null if it doesn't find one with that name
		...
		// description
		//   a meta-model operation that tries to find a state with the same name as aStateName
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that tries to find an event named the same as anEventName
		// requires
		//   none
		// guarantees
		//   (a reference to) the event with the same name as anEventName has been returned
		//     returns null if it doesn't find one with that name
		...
		// description
		//   a meta-model operation that exposes the named transition action, if it's there.
		//     Keep in mind that someone may have renamed something else so we may not be able to find it
		// requires
		...
		// description
		//   a model compiler helper function that returns a list of (input) parameters for
		...
		//   Context.event() <> null
		// guarantees
		//   returns the collection of StateEventBehaviors for the event in Context.event()
		...
		// description
		//   a model compiler helper function that returns the union of all requires clauses
		//     on all Transaction Actions for this Event
		// requires
		//   Context.event() <> null
		// guarantees
		//   if this Event causes any Transitions
		//      then returns an ArrayList of the requires conditions for all TransitionActions on this Event
		...
		// description
		//   a model compiler helper function that returns the union of all guarantees clauses
		//     on all Transaction Actions for this Event
		// requires
		//   Context.event() <> null
		// guarantees
		//   if this Event causes any Transitions
		//      then returns an ArrayList of the guarantees conditions for all TransitionActions on this Event
		//      otherwise returns an empty ArrayList
		...
		// description
		//   a model compiler helper function that tells whether or not there are any state actions
		...
		// description
		//   a PIM Overlay layer helper function that returns all of the imports for this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay layer helper function that returns all of the constants for this class
		// requires
		//   none
		// guarantees
	    ...
	    // description
		//   a PIM Overlay layer helper function that returns all of the helper code for this class
		// requires
		//   none
		// guarantees
	...
	// Modifiers
		...
		// requires
		//   derivedAttributeToAdd <> null and is unique among all derived attributes in this class
		// guarantees
		...
		// description
		//   a meta-model operation that adds a State-Event Behavior to this Meta-Model Class
		...
		// description
		//   a PIM Overlay layer helper function that clears all of the imports for this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay layer helper function that appends the given import statement for this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay layer helper function that clears all of the constants for this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay layer helper function that appends the given constant statement for this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay layer helper function that clears all of the helper code for this class
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay layer helper function that appends the given helper code for this class
		// requires
		//   none
		// guarantees
...
//  Everything from here down is specific to the model compiler itself
		...
		public void ruleJavaClassFile() {
		// description
		//   This is the top-level rule that compiles an entire CIM/PIM class into Java
		//		this rule really only handles the .java file open and close, it doesn't generate any source code
		//
		...
		public void ruleJavaClassFrame() {
		// description
		//   This rule emits the implementation code for a Meta-Model Class
		//     according to the book this is a frame not a rule, but any difference is negligible in practice
		//
		//  Class.#JAVA_CLASS_FRAME -->
		...
		//   for now, ArrayList is the only standard import needed. Can add more later if relevant
		//
		...
		// description
		//   This rule emits the set of instance variables to support all associations this class participates in
		//
		...
		// description
		//   This rule emits initializers (in the constructor) for instance variables that support
		...
		// description
		//   this rule emits the code for getters for all attributes that are not derived attributes
		//		TBD--a smarter rule would only emit getters that were used, an enhancement for later...
		//
		...
		// description
		//   this rule emits code that implements all events (except the constructor) as push operations
		...
		//   none
		// guarantees
		//   Each transition action has been implemented by a (private) operation and method
		...
		//      this rule assumes it doesn't get invoked unless there is at least one state action
		...
		// description
		//   this rule emits the (private) operations and methods that implement each state action
		//      this rule assumes it doesn't get invoked unless there is at least one state action
		//      (i.e., it doesn't need to do the check for "//    none"
		//
		...
		// description
		//   this rule emits code to let anyone access the set of all run-time objects in the class
		//
		//   Class.#All_MEMBERS_ACCESSOR -->
		//		private static ArrayList<ClassName> classNameSet = new ArrayList<ClassName>();
		//
		//		public static ArrayList<ClassName> allClassNames() {
		//		   return classNameSet;
		//		}
		//
		// Note that members are automatically added to classNameSet in generated constructor code
		//   There is, however, no automatic removal. One option is when app client code is doing the
		//   iteration it can ignore objects in the set whose state is "DOESNTEXIST", as in:
		//
		...
		//		}
		//
		//   Another alternative is to include removal code in any destructor / finalizer of this class, as in:
		//
		...
		//   Context.mMClass() = this
		// guarantees
		//   the link and unlink services for association instance variables, if any, have been emitted
			boolean participatesInAnyAssociations = false;
			for( Association anAssociation: Association.allAssociations() ) {
				participatesInAnyAssociations = participatesInAnyAssociations || anAssociation.involvesClass( this );
			}
