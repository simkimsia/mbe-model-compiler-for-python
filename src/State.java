...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// guarantees
		//   returns a populated instance of state (or null, if nothing/error in model file)
	...
	// Accessors
		...
		// description
		//   a compiler operation that formats the state's given name to be usable as a Java ENUM
	...
	//   Everything from here down is specific to the model compiler itself
		...
		//   a call to (private) actions implementing aState's behavior have been emitted
			...
			if( !stateActionSet.isEmpty() ) {
				JavaOutput.indent();
				JavaOutput.println( "if( state == " + this.nameAsQualifiedENUM() + " ) {");
