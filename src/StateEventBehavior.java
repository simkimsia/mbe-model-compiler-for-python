...
//   assumes all risk of use.
	...
	// Constants
		...
		private enum BehaviorType { TRANSITION, IGNORE, IMPOSSIBLE };
	...
	// Static (class) methods
		...
		// guarantees
		//   returns a populated instance of state event behavior (or null, if nothing/error in model file)
	...
	// Constructor(s)
		...
		//   the instance defaults to subclass "Ignore" and must be changed (below) as needed
	...
	// Accessors
		...
		// description
		//   a meta-model operation that returns a reference to the from (i.e., source) state for this StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the string name of the from (i.e., source) state
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns a reference to the event for this StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		public String guard() {
		// description
		//   a meta-model operation that returns the guard text associated with this StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the PIM overlay guard text associated with this StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns whether this is a Transition subtype of StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns whether this is an Ignore  subtype of StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns whether this is an Impossible subtype of StateEventBehavior
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns a reference to the target (to, or destination) state of a Transition subtype
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the name of the target (to, or destination) state of a Transition subtype
		// requires
		//   none
		// guarantees
		//   if this is a Transition subtype
		...
		// description
		//   a meta-model operation that returns the set of Transaction Actions associated with this Transition
		// requires
		//   null
		// guarantees
		//   if this is a Transition subtype
		...
		// description
		//   a meta-model operation that returns the union of all requires clauses on all Transaction Actions for this Transition
		// requires
		//   null
		// guarantees
		//   if this is a Transition subtype
		...
		// description
		//   a meta-model operation that returns the union of all guarantees clauses on all Transaction Actions for this Transition
		// requires
		//   null
		// guarantees
		//   if this is a Transition subtype
		...
		// description
		//   a meta-model operation that returns the union-ed set of parameters associated with this Transition
		// requires
		//   none
		// guarantees
...
// Modifiers
		...
		// description
		//   a meta-model operation that adds another action to the set of transition actions
		// requires
		//   actionToAdd <> null
		// guarantees
		//   if this is a Transition subtype
		...
		// description
		//   a PIM Overlay operation that sets the PIM guard text on a state event behavior
		// requires
		//   guardCondition <> null
		// guarantees
		...
		// description
		//   a meta-model operation that turns this StateEventBehavior into a Transition subtype
		// requires
		...
		// description
		//   a meta-model operation that turns this StateEventBehavior into an Ignore subtype
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that turns this StateEventBehavior into an Impossible subtype
		// requires
		//   none
		// guarantees
...
//   Everything from here down is specific to the model compiler itself
		...
		//      at this point we only need to deal with the remainder: guard, action(s), end state
		...
		//   none
		// guarantees			
		//   proper if-then code is emitted for the given **constructor** State-Event Behavior
		...
		//			Guards are included in the if-then logic when necessary
			if( !this.isIgnore() ) {
				if( this.guard() != "true" ) {
					JavaOutput.println( "if( " + NameService.formatActionStmt( this.pIMGuard() ) + " ) {" );
					JavaOutput.indentMore();
					JavaOutput.indent();
				}
				if( this.isTransition() ) {
					this.ruleTransitionActionsList();
					JavaOutput.println( "state = " + toState.nameAsQualifiedENUM() + ";" );
				} else {
					if( this.isImpossible() ) {
						JavaOutput.println( "throw new IllegalStateException( \"Event is declared impossible in this state\" );" );
					}
				}
				if( this.guard() != "true" ) {
		...
		//			Guards are included in the if-then logic when necessary
			if( !this.isIgnore() ) {
				JavaOutput.print( "if( state == " + fromState.nameAsQualifiedENUM() );
				...
				if( this.isTransition() ) {
					...
					if ( this.fromState() != this.toState() ) {
						...
						if( this.toState() == Context.mMClass().stateNamed( State.defaultNotExistsStateName ) ) {
							JavaOutput.indent();
							JavaOutput.println( NameService.asInstanceLevelName( Context.mMClass().name() ) + "Set.remove( this );" );
						}
					}
					JavaOutput.indentLess();
					JavaOutput.indent();
		...
		// description
		//   This rule outputs a guarantees clause for this behavior (assuming it's not an ignore type)
		//
		...
		//         otherwise fatal exception clause was emitted
			if( !this.isIgnore() ) {
				...
				if( !fromState.name().equals( State.defaultNotExistsStateName ) ) {
					JavaOutput.print( "state was " + fromState.name() );
					if( !this.guard().equals( "true" ) ) {
						JavaOutput.print( " and " );
					}
				}
				if( !this.guard().equals( "true" ) ) {
					JavaOutput.print( this.guard() );
				}
				JavaOutput.print( " --> " );
				if( this.isTransition() ) {
					...
					while( conditionIterator.hasNext() ) {
						...
						JavaOutput.print( aCondition.expression() );
					...
					if( fromState != toState ) {
						...
						JavaOutput.print( "state == " + toState.name() );
					}
				} else {
					JavaOutput.print( "fatal exception thrown " );
				}
			}
		}
		...
		//      otherwise nothing emitted
			if( this.guard() != "true" ) {
				JavaOutput.print( " && " + NameService.formatActionStmt( this.pIMGuard() ) );
			}
		}
	
		public void ruleTransitionActionsList() {
		// description
		//   This rule compiles the set of calls to the (private) actions for a valid Transition
		//
