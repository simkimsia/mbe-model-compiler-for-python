...
//   I decided to use a slightly-modified Java as the action language (Java Action Language = JAL). Here's why:
//     with a few minor modifications to Java to make it an action language, the developer has the full power
//     of Java available in their actions. They can literally do anything that Java can do. The other great
//     benefit is that a developer need not learn yet-another-language (SMALL, TALL, Alf, etc.) to take advantage
//     of model-based development. It also takes a huge load off of this model compiler because now it doesn't
//     have to bother with the action language except for the few minor tweaks. The down side is losing
//     portability that's inherent in those other action languages. They can be compiled into a ton of different
//     concrete languages, Java, C++, C, ... JAL is limited to running on Java platforms unless someone has
//     a handy cross-compiler available.
// Application notes
//   JAL's few tweaks to standard Java are:
//      when you want to use a string, instead of saying "some string", you need to say @ssome string@.
//          be careful, if you say @s some string @ then the Java is " some string ". Everything between
//          the @s and the following @ is considered part of the Java version of the string.
//          (hint, the code @s was used because "s" suggests "String"
//      when you want to refer to an attribute, use @i attribute name @. this will allow JAL and the
//          model compiler to translate the name you see in the model in the same way that the model
//          compiler itself does. the compiler could change underlying naming conventions and the
//          JAL action spec won't be affected in any way.
//          (hint, the code @i was used because "i" suggests "instance level naming conventions"
//      when you want to refer to a class, use @c class name @. again, this allows JAL and the model
//          compiler to translate it into the same internal naming convention and be invisible to the
//          PIM action spec developer
//          (hint, the code @c was used because "c" suggests "Class level naming conventions"
//      along the same lines, using @e some string @, JAL and the compiler will turn it into the appropriately-
//          styled ENUM (e.g., a state name).
//      If you need to do a state comparison in JAL, this is probably the best way for now: assume you are
//          writing JAL action specs for a semantic business model class named "an example class". It has two
//          states, "one state" and "the other state". It also uses the default "state" instance variable.
//          if you need to see if an object is in "one state" in a JAL action spec, you should be able to
//          do it like this
...
//          and this is exactly how the compiler does it in model-generated Java code. there may
//          be a more JAL convenient way to to do it later, but this  at least works for now
//
...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// requires
		//   Input model stream is open and ready to read at the first line after "<action>"
		// guarantees
		//   returns a populated instance of action (or null, if nothing/error in model file)
            ...
            while ( !line.contains( Action.tagActionEnd ) ) {
	            ...
	            if( line.contains( Action.tagPIMReturnDataType ) ) {
	                ...
	                if( newAction.returnRange() == null ) {
	                   System.out.println( " ***** Cannot find declared Range for Action " + newAction.name() + " on class " + Context.mMClass().name() );
	                }
	            }
	...
	// Accessors
		...
		// description
		//   a meta-model operation that returns the description of this Action as a String
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the requires (i.e., pre-condition) set for this Action
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the guarantees (i.e., post-condition) set for this Action
		// requires
		//   none
		// guarantees
		...
		// description
		//   a meta-model operation that returns the defined range (if any) for this Action
		// requires
		//   none
		// guarantees
	...
	// Modifiers
		...
		// description
		//   a meta-model operation that adds the given parameter to the set of parameters on this Action
		// requires
		...
		// description
		//   a meta-model operation that adds the given condition to the requires (i.e., pre-condition) set on this Action
		// requires
		//   conditionToAdd <> null and is unique among all requires Conditions on this Action
		// guarantees
		...
		// description
		//   a meta-model operation that adds the given condition to the guarantees (i.e., post-condition) set on this Action
		// requires
		//   conditionToAdd <> null and is unique among all guarantees Conditions on this Action
		// guarantees
		//   conditionToAdd has been added to the set
		...
		// description
		//   a PIM Overlay operation that clears the action language specification for this Action
		// requires
		//   none
		// guarantees
		...
		// description
		//   a PIM Overlay operation that appends this action language statement to the action language specification for this Action
		// requires
		//   none, null statements should not hurt
		// guarantees
		//   the new ction language statement has been added to the action language spec for this Action
...
//   Everything from here down is specific to the model compiler itself
		...
		//   the code for an invocation (call to) this Action has been emitted
			if( returnRange != null ) {
				JavaOutput.print( " aResult = " );
			}
			JavaOutput.print( "this." + NameService.asInstanceLevelName( name ) );
		...
		// description
		//   this rule emits the code that defines (implements) the action as a private method
		//
		...
		//   none
		// guarantees
		//   the code for the private method implementation of this Action has been emitted
			JavaOutput.indent();
			JavaOutput.print( "private " );
			if( returnRange != null ) {
				JavaOutput.print( returnRange.pIMRunTimeType() + " " );
			} else {
				JavaOutput.print( "void " );
			}
			JavaOutput.print( NameService.asInstanceLevelName( name ) );
		...
		// description
		//   This rule emits the contract for an Action (into the private method definition)
		//
		...
		// description
		//   This rule emits the entry assertions for an Action (into the private method definition)
		//
		...
		// description
		//   This rule emits the PIM Overlay implementation for an Action (into the private method definition)
		//
		//   Action.#ACTION_BODY -->
		...
		//   the PIM Overlay action body for the method has been emitted
			for( String actionStmt: pIMOverlayActionSpec ) {
				JavaOutput.println( NameService.formatActionStmt( actionStmt ) );
				JavaOutput.indent();
			}			
		}
		...
		// description
		//   This rule emits the exit assertions for an Action (into the private method definition)
		//
