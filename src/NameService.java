...
// General description
//   Model compiler
//   This is a mechanism class that provides some name conditioning services to the model compiler
// Invariants
//   none
// Implementation notes
//   it seems really inefficient to do this all in immutable String, but an attempt to use StringBuffer instead
//     led to all kinds of weird characters being injected before converted names. And that led to all kinds of
//     trouble with the code in the IDE. It could not even compile, let alone execute.
// Application notes
//   none
//
...
//		public static String tagDescriptionEnd = "</description>";
	...
	//Static (class) methods
		...
		// description
		//   formats string originalLine to convert non-Java JAL action action language components
		//      into something acceptable to Java:
		//		   1) turns text between @s and @ into quoted text (to support strings and sys.out.print() )
				...
				while( charIndex < originalString.length() && !formattingFailed ) {
					...
					if( theChar != '@' ) {
						...
						switch( theChar ) {
							...
							default:
								System.out.println();
								System.out.println( "****** ERROR in action language statement! '@' followed by inappropriate char ******");
