...
//   assumes all risk of use.
	...
	// Static (class) methods
		...
		// guarantees
		//   returns a populated instance of parameter (or null, if nothing/error in model file)
	...
	// Accessors
		...
		// description
		//   a PIM Overlay operation that returns whether this parameter is assert-able or not
		// requires
		//   none
		// guarantees
	...
	//   Everything from here down is specific to the model compiler itself
		...
		// description
		//   This rule generates the formal parameter name in the signature of the (private) method implementing the Action
		//
		...
		//   none
		// guarantees
		//   the run-time type (from the PIM Overlay) and the parameter name have been emitted
			JavaOutput.print( NameService.formatActionStmt( definedRange.pIMRunTimeType() ) + " " + NameService.asInstanceLevelName( name ) );
		}
		...
		//   the parameter assertion has been emitted
			JavaOutput.println( "assert( " + definedRange.pIMAssertionExpression( NameService.asInstanceLevelName( name ) ) + " );" );
			JavaOutput.indent();
		}
				
		public void ruleActualParameter() {
		// description
		//   This rule generates the actual parameter name in an invocation (call to) the containing Action
		//
